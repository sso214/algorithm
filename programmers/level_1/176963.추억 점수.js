...
function solution(name, yearning, photo) {
  /*
  사진의 추억 점수 : 사진 속에 나오는 인물의 그리움 점수를 모두 합산한 값

  name : 그리워하는 사람의 이름을 담은 문자열 배열
  yearning : 각 사람별 그리움 점수를 담은 정수 배열
  photo : 각 사진에 찍힌 인물의 이름을 담은 이차원 문자열 배열

  result : 사람들의 추억 점수를 photo에 주어진 순서대로 배열에 담아 return
  */
  const arr = new Map(name.map((v,i) => [v,yearning[i]]));
  return photo.map((v) => v.reduce((acc, cur) => acc + (arr.get(cur) || 0), 0));
}
