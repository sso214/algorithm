...
function solution(cards1, cards2, goal) {
    for(let i=0; i<goal.length; i++) {
        const cards1Match = cards1[0] === goal[i];
        const cards2Match = cards2[0] === goal[i];

        if (!cards1Match && !cards2Match) return 'No'
        if (cards1Match) cards1.shift();
        if (cards2Match) cards2.shift();
    }
    return 'Yes';
}
...
/*
알고리즘 문제를 풀 때는 습관적으로 반복문에 for문을 사용했는데
for of 또는 for in 문과 같이 가독성이 좋은 반복문을 주로 사용해야겠다.
*/
