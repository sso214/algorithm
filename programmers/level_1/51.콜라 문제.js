...
/*
Math.trunc : 반올림 없이 순수하게 정수만 표시함 (parseInt 대신 사용해봄)
parseInt : 문자열 인자를 파싱해 특정 진수의 정수를 반환함
~ : 틸트 연산자. 내부적으로 32비트 정수로 변환후 NOT 연산자 실행함 -(n+1)
~~ : ~(Tilt) 사용 시 숫지의 소수점을 버리게 되며, ~~(Tilt)연산을 두번 시킴으로써 결과적으로는 원래 상태로 되돌리게 됨
     양수에서는 Math.floor()와 동일하게 적용되어 결과 값이 같지만 음수에서는 결과값이 달라질 수 있음
*/
